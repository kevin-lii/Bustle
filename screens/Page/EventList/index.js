...
export default class Event extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.navigation.addListener("willFocus", async () => {
      ...
      if (this.state.complete) {
        console.log("update")
        const host = await collection.where("d.host", "==", this.context.uid).orderBy("d.date", "desc").get();
        const tempHost = host.docs.map(doc => ({ id: doc.id, ...doc.data().d }));
        const join = await collection.where("d.invited", "array-contains", this.context.uid).orderBy("d.date", "desc").get();
        const tempJoined = join.docs.filter(doc => doc.data().d.host != this.context.uid).map(doc => ({ id: doc.id, ...doc.data().d }));
        this.setState({ hostedEvents: tempHost, joinedEvents: tempJoined })
      } else if (this.state.joinedEvents.length) {
        const host = await collection.where("d.host", "==", this.context.uid).orderBy("d.date", "desc").get();
        const tempHost = host.docs.map(doc => ({ id: doc.id, ...doc.data().d }));
        const join = await collection.where("d.invited", "array-contains", this.context.uid).orderBy("d.date", "desc").endAt(this.state.lastVisible).get();
        const tempJoined = join.docs.filter(doc => doc.data().d.host != this.context.uid).map(doc => ({ id: doc.id, ...doc.data().d }));
        this.setState({ hostedEvents: tempHost, joinedEvents: tempJoined })
      }
      else {
        const host = await collection.where("d.host", "==", this.context.uid).orderBy("d.date", "desc").endAt(this.state.lastVisible).get();
        const tempHost = host.docs.map(doc => ({ id: doc.id, ...doc.data().d }));
  ...
  retrieveInitialData = async () => {
    try {
      const collection = await EventData.getCollection();
      const host = await collection.where("d.host", "==", this.context.uid).orderBy("d.date", "desc").limit(this.state.limit).get();
      const tempHost = host.docs.map(doc => ({ id: doc.id, ...doc.data().d }));
      if (tempHost.length < this.state.limit) {
        const join = await collection.where("d.invited", "array-contains", this.context.uid).orderBy("d.date", "desc").limit(this.state.limit).get();
        const tempJoined = join.docs.filter(doc => doc.data().d.host != this.context.uid).map(doc => ({ id: doc.id, ...doc.data().d }))
        if (tempJoined.length)
          this.setState({ hostedEvents: tempHost, joinedEvents: tempJoined, lastVisible: join.docs[join.docs.length - 1] })
        else if (tempHost.length)
          this.setState({ hostedEvents: tempHost, lastVisible: host.docs[host.docs.length - 1] })
        else
    ...
    try {
      if (!this.state.complete) {
        ...
        if (!this.state.joinedEvents.length) {
          const host = await collection.where("d.host", "==", this.context.uid).orderBy("d.date", "desc").startAfter(this.state.lastVisible).limit(this.state.limit).get();
          tempHost = host.docs.map(doc => ({ id: doc.id, ...doc.data().d }));
        }
        if (tempHost.length < this.state.limit) {
          const join = await collection.where("d.invited", "array-contains", this.context.uid).orderBy("d.date", "desc").startAfter(this.state.lastVisible).limit(this.state.limit).get();
          const tempJoined = join.docs.filter(doc => doc.data().d.host != this.context.uid).map(doc => ({ id: doc.id, ...doc.data().d }))
          if (tempJoined.length)
            this.setState({
  ...
  render() {
    const changeContext = eventID => {
      ...
      this.context.updateJoinedEvents(this.context.events.filter(item => item != eventID));
    ...
    if (this.state.hostedEvents.length > 0) {
      ...
      data.unshift(
        <Text key={"hosted-title"} style={[styles.subtitle, { alignItems: "center" }]}>
          Hosted Events
      </Text>);
    }
    if (this.state.joinedEvents.length > 0) {
      data.push(<Text key={"joined-title"} style={[styles.subtitle, { alignItems: "center" }]}>
        Joined Events{" "}
        <Icons type="Feather" icon="check-circle" color="green" size={20} />
      </Text>)
      data = [...data,
      this.state.joinedEvents
        .map((event, index) => {
          return (
            <EventDetail
              key={index}
    ...
    return (
      <SafeAreaView style={styles.container}>
        <FlatList
          ...
          ListEmptyComponent={() => !this.state.loading && <Text>You are currently not hosting or joined any events.</Text>}
        ...
        <Modal
          ...
          avoidKeyboard={false}
        >
          <EditEvent close={() => toggleEditEvent(false)} event={this.state.editEvent} {...this.props} />
        </Modal>
      </SafeAreaView>
    );
  }
