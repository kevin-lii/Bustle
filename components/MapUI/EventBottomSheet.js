...
export default class EventBottomSheet extends React.Component {
  ...
  render() {
    ...
    const renderExtended = () => {
      ...
      return (
        <Animated.View style={{ marginTop: animateHeight([15, 5]) }} >
          <Animated.ScrollView style={{ height: Animated.concat(animatedContentHeight, "%") }}>
            <Animated.Text style={[styles.infoTextLast, {
              opacity: animatedContentOpacity,
              lineHeight: animateHeight([20, 0])
            }]}>{event.description}</Animated.Text>
          </Animated.ScrollView>
          <Animated.View style={{
    ...
    return (<BottomSheet
      ...
      onCloseEnd={() =>
        navigateTo({ event: null, events: null })}
    />)
  }
}
